...
use libqaul::{messages::Mode, Qaul, users::UserUpdate};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        router.add_endpoint(
            Arc::try_unwrap(server).unwrap_or_else(|_| panic!("Couldn't get endpoint.")),
        ).await;
        ...
        core.users()
            .update(server_user.clone(), UserUpdate::AddService(IPC_SERVICE_NAME.into()))
            .await?;
        let subscription =
            core.messages()
                .subscribe(server_user.clone(), IPC_SERVICE_NAME, Tag::empty(IPC_SERVICE_TAG)).await?;
