...
impl Dtn {
    ...
    pub fn on_dtn_response(dtn_response: &super::messaging::proto::DtnResponse) {
        ...
        let mut state = match STORAGESTATE.get().write() {
            Ok(s) => s,
            Err(e) => {
                log::error!("DTN: failed to acquire write lock for response: {}", e);
                return;
            }
        };
        if !state.db_ref.contains_key(&dtn_response.signature).unwrap_or(false) {
            return;
        }
        {
            let entry_bytes = match state.db_ref.get(&dtn_response.signature) {
                Ok(Some(bytes)) => bytes,
                _ => {
                    log::error!("DTN: failed to get entry for response");
                    return;
                }
            };
