...
impl RtcManaging{
    ...
    pub fn session_list(_my_user_id: &PeerId)-> super::proto_rpc::RtcSessionListResponse {
        let mut res = super::proto_rpc::RtcSessionListResponse{
            sessions: vec![]
        };
    ...
    pub fn session_request(my_user_id: &PeerId, req: &super::proto_rpc::RtcSessionRequest) -> Result<Vec<u8>, String>{
    ...
    fn send_session_management(my_user_id: &PeerId, conversation_id: &Vec<u8>, option: u32)-> Result<bool, String>{
        //send message on the messaging service
        let proto_message = super::proto_net::RtcContainer {
            message: Some(super::proto_net::rtc_container::Message::RtcSessionManagement(
    ...
    pub fn session_management(my_user_id: &PeerId, req: &super::proto_rpc::RtcSessionManagement) -> Result<Vec<u8>, String>{
        //check if session already exists
        match super::Rtc::get_session_from_id(&req.conversation_id){
            Some(mut session)=>{
                ...
                if let Err(error) = Self::send_session_management(
                    my_user_id, &session.conversation_id.clone(), req.option){
                    return Err(error);
                }
                Ok(session.conversation_id.clone())
            },
            None =>{
                Err("session dese not exists!".to_string())
            }
        }
    ...
    pub fn on_session_request(sender_id: &PeerId, receiver_id: &PeerId, _req: &super::proto_net::RtcSessionRequest, _signature: Vec<u8>){
    ...
    /// process session request from network
    pub fn on_session_management(sender_id: &PeerId, _receiver_id: &PeerId, req: &super::proto_net::RtcSessionManagement, _signature: Vec<u8>){
        ...
        match super::Rtc::get_session_from_id(&sender_id.to_bytes()){
            ...
            None =>{
                log::error!("session management dose not exist session: {}", sender_id.to_string());
            }            
        }
        // save chat messge
