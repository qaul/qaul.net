...
//! Follows the same lazy-init pattern as [`super::ChatStorage`].
...
/// mutable state of chat search indexes
static CHAT_SEARCH: InitCell<RwLock<ChatSearch>> = InitCell::new();

/// Per-account search index manager for chat messages.
pub struct ChatSearch {
    /// search indexes accessible by user account
    indexes: BTreeMap<Vec<u8>, Search>,
}
...
impl ChatSearch {
    /// Initialize the chat search global state.
    /// Must be called once during `Chat::init()`.
    pub fn init() {
        CHAT_SEARCH.set(RwLock::new(ChatSearch {
    ...
    pub fn get_or_create(account_id: &PeerId) -> bool {
        let key = account_id.to_bytes();

        // Check if already open
        {
            let search = CHAT_SEARCH.get().read().unwrap();
            if search.indexes.contains_key(&key) {
                return false;
            }
        }

        // Build the index path: {account_path}/search/chat/
        let account_path = Storage::get_account_path(account_id.to_owned());
        ...
        match Search::new(path_str) {
            Ok(search) => {
                ...
                let mut chat_search = CHAT_SEARCH.get().write().unwrap();
                chat_search.indexes.insert(key, search);
    ...
    pub fn index_message(account_id: &PeerId, item: &impl Searchable) {
        let key = account_id.to_bytes();
        ...
        let mut chat_search = CHAT_SEARCH.get().write().unwrap();
        if let Some(search) = chat_search.indexes.get_mut(&key) {
    ...
    pub fn index_message_batch(account_id: &PeerId, items: &[ChatSearchableMessage]) {
        let key = account_id.to_bytes();
    ...
    pub fn search(
        ...
        let chat_search = CHAT_SEARCH.get().read().unwrap();
        let search = match chat_search.indexes.get(&key) {
            Some(s) => s,
            None => return vec![],
        };
        ...
        match search.search(query, |message_id_b58| {
            Self::reconstruct_result(&account_id_owned, message_id_b58)
        }) {
            Ok(results) => results,
            Err(e) => {
    ...
    fn reconstruct_result(
        ...
        let msg = ChatStorage::get_message_by_id(account_id.to_owned(), &message_id_bytes)?;
