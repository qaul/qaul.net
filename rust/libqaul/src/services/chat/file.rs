...
impl ChatFile {
    ...
    fn get_db_ref(user_id: &PeerId) -> UserFiles {
        ...
        {
            // get chat state
            let all_files = match ALLFILES.get().read() {
                Ok(s) => s,
                Err(e) => {
                    log::error!("Error reading all_files state: {}", e);
                    return Self::create_userfiles(user_id);
                }
            };
    ...
    fn create_userfiles(user_id: &PeerId) -> UserFiles {
        ...
        let mut all_files = match ALLFILES.get().write() {
            Ok(s) => s,
            Err(e) => {
                log::error!("Error writing all_files state: {}", e);
                return user_files;
            }
        };
            ...
            Ok(chatfile) => {
                match chatfile.message {
                    Some(proto_rpc::chat_file::Message::SendFileRequest(send_req)) => {
                        let user_account = match UserAccounts::get_by_id(account_id) {
