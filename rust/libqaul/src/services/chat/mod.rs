...
impl Chat {
    ...
    // send the message
    pub fn send_to_peer(user_account: &UserAccount, chat_message: rpc_proto::ChatMessageSend) -> Result<bool, String> {
        ...
        if chat_message.conversation_id.len() > 16 {
            ...
            let group_id = messaging::ConversationId::from_peers(&user_account.id, &peer_id).unwrap();
        ...
        Self::send_message(&user_account.id, &conversation_id, chat_message.content.clone())
    }


    // send group message
    fn send_message(my_user_id: &PeerId, group_id: &Vec<u8>, message: String )  ->Result<bool, String>{
        let group;
