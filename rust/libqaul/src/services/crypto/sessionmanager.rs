...
impl CryptoSessionManager {
    ...
    fn process_rotate_first(
        ...
        let mut rotate_second = match CryptoNoise::rotate_complete_responder::<
            X25519,
            ChaCha20Poly1305,
            Sha256,
            &[u8],
        ...
        // Declare our final B->A nonce on the old session: it is the
        // nonce this very RotateHandshakeSecond is about to be encrypted
        // with (the next outbound nonce on the draining session), and
        // it is the last thing we ever send on the old session. The
        // initiator drains its inbound side up to this value.
        rotate_second.final_nonce_out = draining_state.index_nonce_out;
    ...
    /// then sending the `RotateHandshakeFinal` cut-over ACK under the
    /// old (now-draining) session. The ACK declares our final A->B
    /// nonce on the old session so the responder can drain it by nonce.
    fn process_rotate_second(
        ...
        // Send the cut-over ACK under the old (draining) session.
        let meta = match crypto_account.get_rotation_meta(*sender_id) {
            Some(m) => m,
            None => return,
        };
        ...
        // Our final A->B nonce on the old session is the nonce this
        // ACK is encrypted with — the last thing we send on it.
        rotate_final.final_nonce_out = draining_state.index_nonce_out;

        let payload = Self::create_rotate_final_message(rotate_final);
        let (encrypted_option, msg_nonce, sess_id, proc_state) = match Crypto::encrypt_with_state(
            payload,
            *sender_id,
            crypto_account.clone(),
            ...
            None => {
                log::warn!(
                    "process_rotate_second: failed to encrypt rotate_final for {}",
                    sender_id.to_base58()
                );
                return;
            }
        ...
        match messaging::Messaging::pack_and_send_encrypted_data(
            ...
            Ok(_) => log::trace!("sent RotateHandshakeFinal to {}", sender_id.to_base58()),
            Err(e) => log::error!("failed sending rotate_final: {}", e),
        }
    }

    /// Handle an incoming `RotateHandshakeFinal` (responder side).
    ///
    /// Records the initiator's declared final A->B nonce as this
    /// node's inbound drain target on the old session, then retires
    /// the draining session if it has already fully drained.
    fn process_rotate_final(
        state: &crate::QaulState,
        user_account: &UserAccount,
        sender_id: &PeerId,
        rotate_final: proto_net::RotateHandshakeFinal,
    ) {
        let crypto_account = CryptoStorage::get_db_ref(state, user_account.id.clone());
        let mut meta = match crypto_account.get_rotation_meta(*sender_id) {
            Some(m) => m,
            None => {
                log::warn!(
                    "process_rotate_final: no rotation meta for {}",
                    sender_id.to_base58()
                );
                return;
            }
        };
        // Tie the ACK to the rotation we just completed as responder.
        if meta.primary_session_id != rotate_final.new_session_id {
            log::warn!(
                "process_rotate_final: new_session_id {} does not match primary {} for {}",
                rotate_final.new_session_id,
                meta.primary_session_id,
                sender_id.to_base58()
            );
            return;
        }
        if meta.draining_session_id.is_none() {
            log::trace!(
                "process_rotate_final: no draining session for {} (already retired)",
                sender_id.to_base58()
            );
            return;
        }
        meta.draining_recv_target = Some(rotate_final.final_nonce_out);
        crypto_account.save_rotation_meta(*sender_id, &meta);

        // The last A->B message may already have arrived — re-check
        // (retires only if inbound-drained AND outbound confirmed).
        Crypto::try_retire_drain(state, &crypto_account, *sender_id);
    }
    ...
    /// Build a `Messaging { CryptoService { content } }` byte payload
    /// carrying a `RotateHandshakeFinal`.
    pub fn create_rotate_final_message(rotate_final: proto_net::RotateHandshakeFinal) -> Vec<u8> {
        let container = proto_net::CryptoserviceContainer {
            message: Some(proto_net::cryptoservice_container::Message::RotateFinal(
                rotate_final,
            )),
        };
        Self::wrap_cryptoservice(container)
