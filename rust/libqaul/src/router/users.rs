...
impl Users {
    ...
    /// The `capabilities` field on the passed-in `user` is NOT persisted —
    /// caps are (re)learned from the next UserInfo advertisement.
        ...
        for signed in signed_profiles {
            match Self::verify_signed_profile(signed) {
                Ok(profile) => {
                    ...
                    let (should_update, verified, blocked, capabilities) = {
                        ...
                        match users.users.get(&q8id) {
                            Some(existing) => {
                                ...
                                (dominated, existing.verified, existing.blocked, existing.capabilities)
                            }
