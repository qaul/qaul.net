...
//! # Upgrade to new version 2.0.0-beta.18
//!
//! Changes to be upgraded:
//!
//! * upgrade sled data base from version 0.29.2 to 0.34.7
//! * change config file peers to use the QUIC protocol for qaul's community server
//!   
...
impl VersionUpgrade {
    ...
    /// upgrade data base
    fn db_upgrade(old_path: &Path, new_path: &Path) -> bool {
        println!("upgrade database folder {:?}", old_path);

        // open old database
        let old_db = sled_old::Config::default()
            .path(old_path)
            .open()
        ...
        // export from old data base to new data base
        let export = old_db.export();
        new_db.import(export);

        println!("old checksum: {:?}", old_db.checksum());
        println!("new checksum: {:?}", new_db.checksum());
    ...
    fn upgrade_config(old_path: &Path, new_path: &Path) -> bool {
        ...
        if let Some(old_cfg) = crate::storage::configuration::Configuration::load(
            ...
            for peer in &old_cfg.internet.peers {
                let peer_address: String;
                if peer.address == "/ip4/144.91.74.192/tcp/9229" {
                    peer_address = "/ip4/144.91.74.192/udp/9229/quic-v1".to_string();
                } else {
    ...
    fn move_content(old_path: &Path, new_path: &Path) -> bool {
        ...
        for entry_res in std::fs::read_dir(old_path).unwrap() {
            ...
            if entry.file_type().unwrap().is_dir() {
                ...
                if file_name.starts_with("12D3Koo") {
                    if !Self::move_content(&old_path.join(file_name), &new_path.join(file_name)) {
                        println!("Error: upgrading user folder {} failed", file_name);
                        return false;
                    }
                    continue;
                }
                if file_name == "node.db" || file_name == "user.db" {
                    if !Self::db_upgrade(&old_path.join(file_name), &new_path.join(file_name)) {
                        println!("Error: upgrading DB {} failed", file_name);
                        return false;
                    }
                    continue;
                }
