...
/// Instance-based state that replaces the former static globals
/// (IGNORE_LIST, DEVICE_LIST, MSG_MAP).
pub struct BleDeviceState {
    ignore_list: Mutex<HashMap<Address, BleScanDevice>>,
    device_list: Mutex<HashMap<Address, BleScanDevice>>,
    msg_map: Mutex<HashMap<String, String>>,
}
...
impl BleDeviceState {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            ...
            msg_map: Mutex::new(HashMap::new()),
        })
    }

    /// Add a new device to list of all previously discovered devices.
    pub fn add_device(&self, device: BleScanDevice) {
        let mut devices = self.device_list.lock().unwrap();
    ...
    pub fn find_device_by_mac(&self, mac_address: Address) -> Option<BleScanDevice> {
        let devices = self.device_list.lock().unwrap();
    ...
    pub fn remove_device_by_mac(&self, mac_address: Address) {
        let mut devices = self.device_list.lock().unwrap();
        devices.remove(&mac_address);
    }

    /// Add a new message to the message map maintained by ble listner.
    pub fn add_msg_map(&self, stringified_addr: String, hex_msg: String) {
        let mut msg_map = self.msg_map.lock().unwrap();
    ...
    pub fn find_msg_map_by_mac(&self, stringified_addr: String) -> Option<String> {
        let msg_map = self.msg_map.lock().unwrap();
        msg_map.get(&stringified_addr).cloned()
    }

    /// Remove a message from the message map maintained by ble listner.
    pub fn remove_msg_map_by_mac(&self, stringified_addr: String) {
        let mut msg_map = self.msg_map.lock().unwrap();
        msg_map.remove(&stringified_addr);
    }

    /// Add a new device to list of devices present nearby and maintain their last found time.
    pub fn add_ignore_device(&self, device: BleScanDevice) {
        let mut ignore_devices = self.ignore_list.lock().unwrap();
    ...
    pub fn find_ignore_device_by_mac(&self, mac_address: Address) -> Option<BleScanDevice> {
        let ignore_devices = self.ignore_list.lock().unwrap();
    ...
    pub fn update_last_found(&self, mac_address: Address) {
        let mut devices = self.ignore_list.lock().unwrap();
    ...
    pub fn remove_ignore_device_by_mac(&self, mac_address: Address) {
        let mut devices = self.ignore_list.lock().unwrap();
        devices.remove(&mac_address);
    }
}
...
pub fn out_of_range_checker(
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            let ignore_list = device_state.ignore_list.lock().unwrap();
            ...
            for (_, device) in ignore_list.iter() {
                ...
                if device.last_found_time != 0 && device.last_found_time < current_time - 5000 {
                    ...
                    device_state.remove_ignore_device_by_mac(mac_address);
                    drop(ignore_list);
                    break;
                } else {
