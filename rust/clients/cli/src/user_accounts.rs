...
impl UserAccounts {
    ...
    pub fn cli(state: &super::CliState, command: &str) {
        match command {
            ...
            // export account
            "export" => Self::handle_export(state, ""),
            cmd if cmd.starts_with("export ") => {
                Self::handle_export(state, cmd.strip_prefix("export ").unwrap().trim());
            }
            // delete account
            "delete" => Self::handle_delete(state, ""),
            cmd if cmd.starts_with("delete ") => {
                Self::handle_delete(state, cmd.strip_prefix("delete ").unwrap().trim());
            }
            // restore account
            cmd if cmd.starts_with("restore ") => {
                Self::handle_restore(state, cmd.strip_prefix("restore ").unwrap().trim());
            }
            // unknown command
    ...
    fn handle_export(state: &super::CliState, args: &str) {
        use libqaul::node::account_management::AccountManagement;

        let parts: Vec<&str> = args.split_whitespace().collect();

        let user_id_base58 = if parts.is_empty() {
        ...
        match AccountManagement::export_account_by_id(
    ...
    ///
    /// If no user_id is given, the default (current) user is used.
    fn handle_delete(state: &super::CliState, args: &str) {
        use libqaul::node::account_management::AccountManagement;
        ...
        match AccountManagement::delete_account_by_id(&state.instance.state, &user_id_base58) {
    ...
    fn handle_restore(state: &super::CliState, args: &str) {
        use libqaul::node::account_management::AccountManagement;
        ...
        match AccountManagement::restore_account_from(&state.instance.state, archive_path) {
