...
data class ReceivedMessage(
    ...
    val largeMessageIndicator: Byte,
...
class ReceiveQueueResult {
    ...
    /** Internal use by ReceiveQueue only. Set by assembleMessage when a large-message part's
...
class ReceiveQueue {
    ...
    fun incomingMessage(chunk: ByteArray, device: BluetoothDevice): ReceiveQueueResult {
        // (was: per-chunk toBinaryString(chunk) feeding the commented log below — dead hot-path work)
        //AppLog.e(TAG, "GattMessaging incomingMessage: chunk: ${BLEUtils.toBinaryString(chunk)}")
        ...
        // Hot path: fires on every incoming chunk. Per-chunk logcat I/O throttles receive
        // throughput (same effect the per-write log had on the sender). Re-enable for debugging only.
        ...
        if (receiveMessageStructure.queueIndex == 0x00.toByte()) {
            ...
            // assembleMessage now uses assembledLargeMessagePart for
            // large message parts, so receivedMessage is only ever set here for normal messages.
            // For large message parts, feed the assembled part into the collector,
            // receivedMessage will be set only when every part has arrived.
            val assembledPart = receiveQueueResult.assembledLargeMessagePart
            if (assembledPart != null) {
                receiveQueueResult.receivedMessage = addLargeMessagePart(assembledPart)
            }
        }
    ...
    private fun messageHeader(chunk: ByteArray): ReceiveMessageStructure {
        // (removed two per-chunk BLEUtils.toBinaryString() calls here — the result was never used)
        ...
        if (type == 0x00.toByte()) {
            ...
            chunkIndex = ((b1.toInt() and 0x03 shl 8) + (b2.toInt() and 0xFF)).toShort()
    ...
    private fun incomingMessageChunk(receiveMessageStructure: ReceiveMessageStructure): ReceiveQueueResult {
        // Hot path: fires on every data chunk. Both the log and the toBinaryString() conversion
        // (which only fed the commented log below) are per-chunk costs that throttle receive
        // throughput. Re-enable for debugging only.
        ...
        //val binaryString = BLEUtils.toBinaryString(receiveMessageStructure.payload)
    ...
    private fun addLargeMessagePart(receivedMessage: ReceivedMessage): ReceivedMessage? {
        ...
        val rawPartsTotal = (largeMessageIndicator.toInt() shr 2) and 0x03
        val partsTotal = if (rawPartsTotal == 0) 4 else rawPartsTotal // TODO: Check protocol spec for how partsTotal should be calculated for 00
        ...
        if (receiveLargeMessage == null) {
            ...
            // Capture when the first part of this large message arrived for receiver side metrics
            receiveLargeMessage.createdAt = receivedMessage.createdAt
        }
        ...
        if (allPartsReceived) {
            ...
            for (i in 0 until partsTotal) {
                val part = receiveLargeMessage.messageParts[i]
                if (part == null) {
                    AppLog.e(TAG, "addLargeMessagePart: part $i is null despite allPartsReceived=true, aborting assembly")
                    return null
                }
                finalMessage += part
            }
            ...
            receivedMessage = ReceivedMessage(
                ...
                finalMessage,
                0x00.toByte(),
                receiveLargeMessage.createdAt  // start time of the first chunk of the first part
            )
        }
    ...
    private fun assembleMessage(receiveQueueResult: ReceiveQueueResult): ReceiveQueueResult {
        ...
        if (!error) {
            ...
            if (crc32Value != crc32Checksum) {
                ...
                if (largeMessageIndicator == 0x00.toByte()) {
                    // Normal (non-large) message — fully deliverable right now.
                    receiveQueueResult.receivedMessage = ReceivedMessage(qaulId, message, largeMessageIndicator, createdAt)
                } else {
                    // One part of a large multi-part message — not deliverable yet.
                    // ReceiveQueue.incomingMessage will forward this to addLargeMessagePart.
                    receiveQueueResult.assembledLargeMessagePart = ReceivedMessage(qaulId, message, largeMessageIndicator, createdAt)
                }
            }
        }
...
class ReceiveLargeMessage {
    ...
    // Timestamp of the first chunk of the first part — used for receive-side metrics.
    var createdAt: kotlin.time.TimeSource.Monotonic.ValueTimeMark = kotlin.time.TimeSource.Monotonic.markNow()
